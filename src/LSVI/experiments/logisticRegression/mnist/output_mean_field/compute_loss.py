...
from LSVI.experiments.logisticRegression.mnist.load_mnist import mnist_dataset
from LSVI.experiments.logisticRegression.utils import get_tgt_log_density
from LSVI.variational.exponential_family import MeanFieldNormalDistribution, GenericMeanFieldNormalDistribution
from LSVI.variational.utils import gaussian_loss
...
if __name__ == "__main__":
    ...
    for idx, my_pkl in enumerate(PKLs):
        if PKL_titles[idx] not in EXCLUDED_PICKLES:
            if not os.path.exists(f"{OUTPUT}/{PKL_titles[idx][:-4]}_loss.pkl") and not "blackjax" in PKL_titles[idx]:
