...
class Inference:
    ...
    def fit(
        ...
        self.means = np.empty(shape=(n, self.approx.ddim))
        self.covs = np.empty(shape=(n, self.approx.ddim, self.approx.ddim))
        ...
        self.state = state
        self.approx.means = means
        self.approx.covs = covs
    ...
    def _iterate_with_loss(self, s, n, step_func, progressbar, progressbar_theme, callbacks):
        ...
        try:
            with CustomProgress(
                ...
                for i in range(n):
                    e = step_func()
                    progress.update(task, advance=1)
                    current_gaussian = self.approx.mean.eval(), self.approx.cov.eval()
                    self.means[i] = current_gaussian[0]
                    self.covs[i] = current_gaussian[1]
                    current_param = self.approx.params[0].get_value()
                    if np.isnan(e):
                        scores = scores[:i]
        ...
        self.hist = np.concatenate([self.hist, scores])
        return State(i + s, step=step_func, callbacks=callbacks, score=True), self.means, self.covs
