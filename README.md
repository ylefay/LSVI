...
a easy-to-sample from distribution, to estimate posterior-based quantities.

# How it's done in this package?

The first-order optimality condition $\nabla_{\eta}KL(q_\eta\mid \pi)$ with $\pi \propto \exp(f(x))$ and $q_{\eta}(x) = \exp(\eta^{\top}s(x))$ suggests the
following scheme

```math
\eta_{t+1} = (1-\varepsilon_t) \eta_t+\varepsilon_t \textup{argmin}_{\eta\in \mathbb{R}^m} \mathbb{E}_{\eta_t}(\eta^{\top}s(X) - f(X))^2,
```
...
When the variational family is the set of full-rank Gaussian distribututions, the previous iteration requires inversion of $n^2 \times n^2$ matrices.
