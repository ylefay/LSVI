...
def time_heuristic_gaussianMeanField_Nicolas_u10_fixed1em3():
    ...
    heuristic_gaussianMeanField_Nicolas(n_samples=int(n_samples), n_iter=n_iter, lr_schedule=Seq,
...
def time_gaussianMeanField_Nicolas_fixed1em3():
    ...
    n_samples = 1e4
    OP_key = jax.random.PRNGKey(0)
    gaussianMeanField_Nicolas(n_samples=int(n_samples), n_iter=n_iter, lr_schedule=Seq, title_seq=Seq_title,
                              OP_key=OP_key, OUTPUT_PATH=OUTPUT_PATH)
...
def time_ngd_diagonal():
    ...
    OP_key = jax.random.PRNGKey(0)
    ngd_diagonal(OP_key, n_iter, n_samples, lr, OUTPUT_PATH=OUTPUT_PATH)
