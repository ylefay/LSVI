    ...
    for idx, my_pkl in enumerate(PKLs):
        if PKL_titles[idx] not in EXCLUDED_PICKLES and "blackjax" in PKL_titles[idx]:
            if not os.path.exists(f"{OUTPUT}/{PKL_titles[idx][:-4]}_loss.pkl"):
                size_pkl = my_pkl['states'][0].shape[0]
                ...
                for k in range(size_pkl // SIZE_vmap):
                    ...
                    c1 = my_pkl['states'][0][k * SIZE_vmap:min((k + 1) * SIZE_vmap, size_pkl)]
                    c2 = my_pkl['states'][1][k * SIZE_vmap:min((k + 1) * SIZE_vmap, size_pkl)]
                ...
                if size_pkl % SIZE_vmap != 0:
                    ...
                    c1 = my_pkl['states'][0][-(size_pkl % SIZE_vmap):]
                    c2 = my_pkl['states'][1][-(size_pkl % SIZE_vmap):]
