...
def lsvi(sampling, sufficient_statistic, tgt_log_density, eta_init, n_iter, n_samples, lr_schedule=1.0,
    ...
    if return_all:
        ...
        _sample = sampling(eta_init[:-1], n_samples)
    ...
    for i_iter in range(n_iter):
        ...
        next_eta = np.linalg.lstsq(X, y, rcond=None)[0]
        lr, residual = momentum_backtracking(lr, current_eta, next_eta, y, X, target_residual)
        next_eta = next_eta * lr + (1 - lr) * current_eta
        etas[i_iter + 1] = next_eta
        if return_all:
            ys[i_iter] = y
            Xs[i_iter] = X
